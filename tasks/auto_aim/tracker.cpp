...
//P0_dig是 “初始状态协方差矩阵P0的对角线元素
//协方差矩阵P0的基础意义:1:对角线元素P0(i,i)：表示状态向量中第i个维度的方差（不确定性大小），值越大，说明该状态的初始估计越不可靠
                    //:2:非对角线元素P0(i,j)：表示第i和j个状态的协方差（相关性），通常初始时假设状态间不相关，非对角线元素设为 0
  if (is_balance) {     //平衡步兵，时代的旧衣物
    Eigen::VectorXd P0_dig{{1, 64, 1, 64, 1, 64, 0.4, 100, 1, 1, 1}}; //11维，开根号1，8，1，8，1，8....（前六个为xyz的位置方差和速度方差+-1m，+-8m/s）为方差
